...
function run_yad(cmd)
  ...
  if #lines == 0 then return nil, code end
...
function _create_bookmark_layer(name)
  ...
  local final_name = (type(name) == "string" and name ~= "")
                      and name
                      or ("Bookmark_" .. os.date("%H%M%S"))
...
function search_bookmark_next()     search_bookmark(1)  end
function search_bookmark_prev()     search_bookmark(-1) end
    ...
    for i, part in ipairs(parts) do
      if not current[part] then
        current[part] = {
          ...
          first_page = bm.page,   -- used for folder page hint
        }
      end
      if i == #parts then
      ...
      -- keep first_page as the earliest page seen under this node
  ...
  local function traverse(node, indent)
    ...
    for _, name in ipairs(keys) do
      ...
      if data.bookmark then
        ...
        -- folder node: show page of first child as hint
        table.insert(flat_list, {
...
function view_bookmarks()
  ...
  for _, item in ipairs(flat_list) do
    cmd = cmd .. string.format(" %s %s %s",
      bash_escape(item.id),
      bash_escape(tostring(item.page)),
  ...
  if not selected_id or selected_id:sub(1, #FOLDER_ID_PREFIX) == FOLDER_ID_PREFIX then return end
  ...
  elseif code == YAD_EDIT then
    local form_cmd = string.format(
      'yad --form --title="Edit Bookmark"'
      .. ' --field="Page:NUM" "%d..%d..1"'
      .. ' --field="Name" %s',
      selected.page,
      #app.getDocumentStructure().pages,
...
function export_bookmarks_to_file()
  ...
  local filename, code = run_yad(
    'yad --file --save --confirm-overwrite --title="Export Bookmarks"'
    .. ' --filename=' .. bash_escape(os.tmpname() .. "_bookmarks.json"))
  if code ~= YAD_OK or not filename then return end
...
function parse_bookmarks_json(content)
  ...
  for block in content:gmatch("{([^}]+)}") do
    ...
    local name_str = block:match('"name"%s*:%s*"(.-[^\\])"')
                  or block:match('"name"%s*:%s*""')  -- if empty name
    if page_str and name_str then
      -- unescape \" inside the name
      name_str = name_str:gsub('\\"', '"')
      table.insert(results, { page = tonumber(page_str), name = name_str })
    end
  end
  return results
end
...
function export_import_bookmarks()
  local out, code = run_yad(
    'yad --list --title="Import / Export" --width=300 --height=200'
    .. ' --column="Action" "Export Bookmarks" "Import Bookmarks"'
    .. ' --button="OK:0" --button="Cancel:1"')
  if code ~= YAD_OK or not out then return end
...
function export()
  ...
  local ok, err = pcall(function()
    ...
    if not exec_ok("pdftk " .. bash_escape(tempPdf) ..
        " dump_data output " .. bash_escape(tempData)) then
      error("pdftk dump_data failed")
    end

    -- append bookmarks
    local file = io.open(tempData, "a+")
    if not file then error("Cannot open temp metadata file: " .. tempData) end
    ...
    if not exec_ok("pdftk " .. bash_escape(tempPdf) ..
        " update_info " .. bash_escape(tempData) ..
        " output " .. bash_escape(path)) then
